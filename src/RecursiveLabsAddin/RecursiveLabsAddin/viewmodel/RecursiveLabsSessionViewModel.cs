...
{
    public class RecursiveLabsSessionViewModel : ViewModelBase
    {
        ...
        public RecursiveLabsSessionType SessionType
        {
            get { return SimpleSessionType ? RecursiveLabsSessionType.Host : RecursiveLabsSessionType.View; }
        }
        ...
        public RecursiveLabsSessionViewModel(Interaction interaction)
        {
            _client = new RestClient(ApiUrl);
            _interaction = interaction;

            // Get guest name
            if (!string.IsNullOrEmpty(RecursiveLabsButton.GuestNameAttribute))
                _guestName = _interaction.GetStringAttribute(RecursiveLabsButton.GuestNameAttribute);
            if (string.IsNullOrEmpty(_guestName))
                _guestName = !string.IsNullOrEmpty(RecursiveLabsButton.GuestName) ? RecursiveLabsButton.GuestName : "Guest";

            // Get guest email
            if (!string.IsNullOrEmpty(RecursiveLabsButton.GuestEmailAttribute))
                _guestEmail = _interaction.GetStringAttribute(RecursiveLabsButton.GuestEmailAttribute);
            if (string.IsNullOrEmpty(_guestEmail))
                _guestEmail = !string.IsNullOrEmpty(RecursiveLabsButton.GuestEmail) ? RecursiveLabsButton.GuestEmail : "fakeguest@fakedomianname.com";
        }
        ...
        private T Execute<T>(Method method, string resource, params Parameter[] parameters) where T : new()
        {
            ...
            request.AddHeader("Authorization", RecursiveLabsButton.Authorization);
        ...
        private void SendLinkToChat()
        {
            try
            {
                ...
                if (SessionType == RecursiveLabsSessionType.View)
                {
            ...
            try
            {
                ...
                var response = Execute<InviteToHostResponse>(Method.GET, "/session/inviteToHost",
                    new Parameter
                    {
                        ...
                        Name = "hostName",
                        Value = SessionType == RecursiveLabsSessionType.Host ? RecursiveLabsButton.AgentName : _guestName
                    },
                    new Parameter
                    {
                        Type = ParameterType.GetOrPost,
                        Name = "hostEmail",
                        Value = SessionType == RecursiveLabsSessionType.Host ? RecursiveLabsButton.AgentEmail : _guestEmail
                    },
                    new Parameter
                    {
                        Type = ParameterType.GetOrPost,
                        Name = "guestName",
                        Value = SessionType == RecursiveLabsSessionType.Host ? _guestName : RecursiveLabsButton.AgentName
                    },
                    new Parameter
                    {
                        Type = ParameterType.GetOrPost,
                    ...
                    new Parameter
                    {
                        Type = ParameterType.GetOrPost,
                        Name = "screenDomain",
                        Value = RecursiveLabsButton.ScreenDomain
                    });
                ...
                Process.Start(SessionType == RecursiveLabsSessionType.Host
                    ? HostLink
                    : GuestLink);
